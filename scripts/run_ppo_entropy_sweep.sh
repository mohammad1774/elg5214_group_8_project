...
#
# Usage:
#   bash scripts/run_ppo_entropy_sweep_cpu.sh
#   bash scripts/run_ppo_entropy_sweep_cpu.sh --parallel 2
#   bash scripts/run_ppo_entropy_sweep_cpu.sh --parallel 4
# ============================================================================
        ...
        for lr in "${LRS[@]}"; do
            for alpha in "${ALPHAS[@]}"; do
                for seed in "${SEEDS[@]}"; do
                    ...
                    OUTPUT_FILE="results/ppo_entropy/${env_name}_${reward_type}/lr${lr}_g${gamma}_a${alpha}_seed${seed}.csv"
                    if [ -f "$OUTPUT_FILE" ]; then
                        SKIPPED=$((SKIPPED + 1))
                        continue
                    fi

                    COMMANDS+=("python -m src.test.test_ppo_entropy_agent --seed $seed --lr $lr --gamma $gamma --alpha $alpha --env $env_name --reward $reward_type --config $CONFIG")
                done
            done
        done
    done
...
echo "  Results in: results/ppo_entropy/"
echo "============================================"
