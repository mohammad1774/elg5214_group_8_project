"""
test_dqn_entropy_agent.py — Run a single DQN+Entropy training run on CPU.
...
# Extra safety: tell JAX again to use CPU
...
def run(args):
    ...
    logger.info(
        f"DQN+Entropy | seed={args.seed}, lr={args.lr}, gamma={args.gamma}, alpha={args.alpha}"
    )
    logger.info(f"Env: {args.env}/{args.reward}, obs_dim={obs_dim}, num_actions={num_actions}")
    logger.info(f"Forced JAX backend: {jax.default_backend()}")
    ...
    print(
        f"[DQN+Entropy | {args.env}/{args.reward} | s={args.seed} lr={args.lr} "
...
def main():
    parser = argparse.ArgumentParser(description="DQN + Entropy single run (CPU)")
