"""
test_ppo_entropy_agent.py — Run a single PPO+Entropy training run.
...
def run(args):
    ...
    log_dir = config.get("log_dir", "./logs/ppo_entropy")
    results_dir = config.get("results_dir", "results/ppo_entropy")
    project_name = config.get("project_name", "PPO_Entropy")
    ...
    logger.info(
        f"PPO+Entropy | seed={args.seed}, lr={args.lr}, "
    ...
    timer = Timer("PPO+Entropy training")
    with timer:
    ...
    print(
        f"[PPO+Entropy | {args.env}/{args.reward} | s={args.seed} lr={args.lr} "
        f"g={args.gamma} a={args.alpha}] success={eval_stats['success_rate']:.3f}, "
...
def main():
    parser = argparse.ArgumentParser(description="PPO + Entropy single run")
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/ppo_entropy.yaml",
    )
