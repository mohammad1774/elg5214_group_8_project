"""
train_ppo_rnd.py — PPO with Random Network Distillation training.
...
def train_ppo_rnd(
    ...
    for episode in range(1, num_episodes + 1):
        ...
        agent = PPORNDAgent(policy_params, num_actions=num_actions)
        ...
        intrinsic = compute_rnd_reward_batch(rnd_params, obs)
        augmented_rewards = rewards + beta * intrinsic
        values = value_forward_batch(value_params, obs)
        last_value = value_forward(value_params, rollout["next_observations"][-1])
        advantages = _compute_gae(augmented_rewards, values, dones, last_value, gamma, gae_lambda)
        ...
        rnd_params, rnd_loss = update_rnd_predictor(rnd_params, obs, predictor_lr)
