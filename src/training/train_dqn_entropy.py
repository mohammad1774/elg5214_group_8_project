"""
train_dqn_entropy.py — DQN + Entropy Regularization training.

WHAT CHANGES VS BASELINE DQN (train_dqn.py):
    The loss function adds an entropy bonus:

        L_total = L_td - alpha * H(softmax(Q(s,a)))

    L_td is the standard TD error (same as baseline DQN).
    H() is the entropy of the soft policy induced by Q-values.
    alpha controls the strength of the entropy bonus.

    By subtracting alpha * entropy from the loss, we MAXIMIZE entropy,
    which keeps the Q-values spread out and prevents premature
    convergence to a single dominant action.

    Everything else is identical to baseline DQN:
    - Same epsilon-greedy action selection
    - Same replay buffer
    - Same target network updates
    - Same lax.scan episode collection
...
from src.exploration.entropy_reg import entropy_from_q_values
...
def dqn_entropy_loss(q_params, target_params, batch, gamma=0.99, alpha=0.01):
    """Standard TD loss MINUS entropy bonus.

    L = mean((Q(s,a) - target)^2) - alpha * mean(H(softmax(Q(s,:))))

    The entropy term encourages Q-values to stay spread across actions.
    """
    # Standard TD error
    ...
    td_loss = jnp.mean((q_sa - targets) ** 2)

    # Entropy bonus: H(softmax(Q)) for each state in the batch
    entropy_per_state = jax.vmap(entropy_from_q_values)(q_values)
    mean_entropy = jnp.mean(entropy_per_state)

    # Subtract entropy from loss → maximizes entropy
    total_loss = td_loss - alpha * mean_entropy

    return total_loss, mean_entropy


@partial(jax.jit, static_argnames=("gamma",))
def update_q_network_entropy(q_params, target_params, batch,
                             learning_rate, gamma=0.99, alpha=0.01):
    """One gradient step with entropy-augmented loss."""
    loss_fn = lambda p: dqn_entropy_loss(p, target_params, batch, gamma, alpha)[0]
    ...
    # Also compute entropy for logging (no grad needed)
    _, mean_entropy = dqn_entropy_loss(new_params, target_params, batch, gamma, alpha)

    return new_params, loss, mean_entropy


# ──────────────────────────────────────────────
#  lax.scan episode collection (same as baseline DQN)
# ──────────────────────────────────────────────
...
def train_dqn_entropy(
    ...
    for episode in range(1, num_episodes + 1):
        ...
        # 2. Insert transitions into buffer
        buffer = add_transitions_batch(buffer, rollout, n_valid)
        global_step += n_valid

        # 3. Gradient updates with entropy bonus
        ...
        if int(buffer["size"]) >= max(warmup_steps, batch_size):
            ...
            for _ in range(n_updates):
                ...
                q_params, loss, mean_ent = update_q_network_entropy(
                    q_params=q_params,
                ...
                total_entropy = total_entropy + mean_ent
        ...
        avg_entropy = float(total_entropy / max(n_updates, 1))
        ...
        logger.info(
            ...
            f"Entropy: {avg_entropy:.4f}, Epsilon: {epsilon:.3f}"
        )
        met_df.add_episode(
            seed=seed, episode=episode, reward=ep_reward,
            episode_length=ep_length, loss=avg_loss,
            algorithm="DQN_Entropy", lr=learning_rate, gamma=gamma,
    ...
    return {
        ...
        "entropies": jnp.array(entropies, dtype=jnp.float32),
